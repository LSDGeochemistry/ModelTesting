...
#Import the data_
DataDirectory = '/exports/csce/datastore/geos/users/s0933963/github/LSDMixingModel/Runs/feather_runs/pomd/linear_mixing_ss_new_lowest_erate/'
...
cosmo_data = pd.read_csv('/exports/csce/datastore/geos/users/s0933963/feather_river_mixing_paper/Cosmo_spreadsheets/pomd_cosmo.csv',sep=",",header=0,names=['Sample_Code','Pit','DS_dist','Start_d','End_d','Horizon','Be_conc','Be_error'] )
cosmo_pits = cosmo_data.Pit.unique()
cosmo_pits = np.array(cosmo_pits)
cosmo_pits = cosmo_pits.astype(np.float)


pit_data = pd.read_csv('/exports/csce/datastore/geos/users/s0933963/github/ModelTesting/flowtube_testing/pomd_sites_csv.txt',sep=",",header=0,names=['X_coord','Y_coord','PIT_REF','elev','soil_thick'] )

pit_data['PIT_REF'] = pit_data['PIT_REF'].astype(str).str.replace('POMD-','').astype(float)
...
DataDirectory = '/exports/csce/datastore/geos/users/s0933963/github/LSDMixingModel/Runs/feather_runs/pomd/nonlinear_mixing_ss_new_lowest_erate/'
print('This is the input file directory: '+DataDirectory)
#Load all the data
print('Load the flowtube data')
ft_out = pd.read_csv(DataDirectory+'ft_properties.out', sep=" ",header=0,names=['s', 'b', 'A', 'zeta', 'eta', 'h'] )
...
# plt.show()
# plt.tight_layout()
plt.savefig(DataDirectory+'pomd_crn_pits_combined_no_mixing.png', dpi=100, bbox_inches='tight')
