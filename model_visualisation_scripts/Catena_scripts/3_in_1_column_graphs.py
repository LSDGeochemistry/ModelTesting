...
#Import the data_
DataDirectory = '/exports/csce/datastore/geos/users/s0933963/github/LSDMixingModel/Runs/changing_base_level/0_1mm_linear/'

print('This is the input file directory: '+DataDirectory)
folname = '0mm_mixing_full_cosmo'
#Load all the data
print('Load the flowtube data')
ft_out = pd.read_csv(DataDirectory+folname+'/ft_properties.out', sep=" ",header=0,names=['s', 'b', 'A', 'zeta', 'eta', 'h'] )
print('Load the eroded particle data')
eroded_bins = pd.read_csv(DataDirectory+folname+'/ep_trans_out.pout', sep=" ",names=['time', 'bn', 'pid','z_loc','s_loc','d_loc','buff','page','osl','be_conc','fallout_be_conc','c_conc','ne_conc'] )
print('Load the particle data')
bins = pd.read_csv(DataDirectory+folname+'/p_trans_out.pout', sep=" ",names=['time', 'bn', 'pid','z_loc','s_loc','d_loc','buff','page','osl','be_conc','fallout_be_conc','c_conc','ne_conc'] )
...
ax.axhline(y=start_be_mean,color="black",linestyle="--",label='Old Steady State')
ax.axhline(y=end_be_mean,color="black",linestyle=":",label='New Steady State')
ax.legend(loc=4)
...
plt.savefig(DataDirectory+'/changing_base_level_column_averages.png', dpi=100, bbox_inches='tight')