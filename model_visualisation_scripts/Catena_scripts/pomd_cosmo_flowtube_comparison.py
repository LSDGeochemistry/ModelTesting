...
#     b = d
#     xb = d
#     # tot =(p/(e*l**-1+lm))*((e+lm*l*(1-np.exp(-xb*l**-1)))/e+lm*xb)*scaling[i]
#     # full_mix_tot = full_mix_tot+tot
...
for i in range(0,len(time_unique)):
    ...
    for j in range(0,len(bin_index)):
        ...
        ax.scatter(temp_cosmo.Be_conc,(temp_cosmo.Start_d+temp_cosmo.End_d)/2/100,s=5.0)
        ...
        plt.axvline(full_mix_tot,c='grey',alpha=0.5,ls='-.',label='POMD mixing erosion rate')
...
plt.savefig(DataDirectory+'/pomd_crn_pits.png', dpi=100, bbox_inches='tight')
