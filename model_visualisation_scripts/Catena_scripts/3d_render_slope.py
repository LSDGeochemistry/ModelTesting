...
for j in range(0,1):
    ...
    for i in range(1,2):
        thick = ft_data[i*pits:i*pits+pits][:,5]
        thick = [x for item in thick for x in repeat(item,2)]
        temp_bins1 = bins[(bins['time'] == time_unique[j]) & (bins['bn'] >= (i)) & (bins['bn'] <= (i)+2) & (bins['d_loc'] <= thick[i]+3.0)]
        ...
        for idx, row in temp_bins1.iterrows():
            ...
            cb = ax.scatter(s_loc,np.random.random()*1,z_loc,c=row.be_conc,s=0.1,cmap='viridis',vmin=min,vmax=max,zorder=1)   
...
for j in range(0,1):
    ...
    for i in range(1,2):           
        ...
        if i == 1:
            ax.plot([ft_data_temp[i-1,0],ft_data_temp[i-1,0]],[0,0],[ft_data_temp[i-1,3],ft_data_temp[i-1,4]],c='k',zorder=3)
            ax.plot([ft_data_temp[i-1,0],ft_data_temp[i-1,0]],[0+ft_data_temp[i-1,1],0+ft_data_temp[i,1]],[ft_data_temp[i-1,3],ft_data_temp[i-1,4]],c='k',zorder=3)
            ax.plot([ft_data_temp[i-1,0],ft_data_temp[i-1,0]],[0,0+ft_data_temp[i-1,1]],[ft_data_temp[i-1,3],ft_data_temp[i-1,3]],c='k',zorder=3)
            ax.plot([ft_data_temp[i-1,0],ft_data_temp[i-1,0]],[0,0+ft_data_temp[i,1]],[ft_data_temp[i-1,4],ft_data_temp[i-1,4]],c='k',zorder=3)

            
        if i == 2:
            ax.plot([ft_data_temp[i,0],ft_data_temp[i,0]],[0,0],[ft_data_temp[i,3],ft_data_temp[i,4]],c='k',zorder=3)
            ax.plot([ft_data_temp[i,0],ft_data_temp[i,0]],[0+ft_data_temp[i-1,1],0+ft_data_temp[i,1]],[ft_data_temp[i,3],ft_data_temp[i,4]],c='k',zorder=3)
            ax.plot([ft_data_temp[i,0],ft_data_temp[i,0]],[0,0+ft_data_temp[i,1]],[ft_data_temp[i,3],ft_data_temp[i,3]],c='k',zorder=3)
            ax.plot([ft_data_temp[i,0],ft_data_temp[i,0]],[0,0+ft_data_temp[i,1]],[ft_data_temp[i,4],ft_data_temp[i,4]],c='k',zorder=3)
