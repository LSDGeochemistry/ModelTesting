...
bins = pd.read_csv(DataDirectory+'p_trans_out.pout', sep=" ",names=['time', 'bn', 'pid','z_loc','s_loc','d_loc','buff','page','osl','be_conc','c_conc','ne_conc'] )
...
for i in range(0,len(time_unique)):
    ...
    for j in range(0,len(bins_unique)//2):
        ...
        std_dv = temp_bins.std().c_conc
        file.write(str(sample) + ' ' + 'C-14 ' + ' ' + str(mineral) + ' ' + str(mean_crn) + ' ' + str(std_dv) +';'+'\n')
