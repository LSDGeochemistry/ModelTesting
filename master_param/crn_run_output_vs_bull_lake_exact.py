...
for subdirs, dirs, files in os.walk(root):
    for dirs in dirs:
        ...
        for i in range(0,len(bins)-1):
            temp = data[(data.d_loc >= bins[i][0]) & (data.d_loc <= bins[i][1])]
        ...
        #Find the mean values of each bi
            ...
            cb = ax.scatter(mean_be,mean_d,s=20,color=colors[counter])
            ax.errorbar(mean_be,mean_d,xerr=be_std,yerr=d_std, fmt='none',color=colors[counter])
